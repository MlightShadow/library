...
用户态线程不主动放弃cpu使用权，该进程中其他线程是无法被执行的。
...
公平：进程等待时间，占用cpu的时长上要公平


#### 调度算法的指标

* cpu使用率：cpu忙状态所占时间的百分比；
* 吞吐量：单位时间内完成的进程数量；
...
在SPN的基础上更加关注进程的等待时间，防止无限期推迟，抢占支持较差
...
低优先级进程使用某种公共资源导致高优先级进程无法继续，从而使得其他次级优先执行的进程优于高优先级进程先被执行从而抢占CPU资源
